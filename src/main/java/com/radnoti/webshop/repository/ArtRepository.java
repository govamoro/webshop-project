...
public interface ArtRepository extends CrudRepository<Art, Integer> {
    ...
    @Modifying
    @Query("UPDATE Art a SET a.basket.id = NULL WHERE a.basket.id = :basketId")
    void deleteWholeBasket(Integer basketId);


    @Modifying
    @Query("update Art a SET a.basket.id = null where a.id = :artId")
    void removeArtFromBasket(Integer artId);
}
